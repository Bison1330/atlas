...
| M7  | Basic auth           | in progress | Branch `m7/basic-auth`. **Thin slice: email+password + drawing ownership** — `users` table, nullable `owner_id` on `drawings`, `POST /drawings/{id}/claim` for legacy data. Auth primitives: argon2id hashing, Redis-backed session cookies (HttpOnly/Secure/SameSite=Lax, 14d sliding TTL, opaque IDs signed with itsdangerous), double-submit CSRF cookie, rate-limited login (5/15m) + register (3/hr). Every drawing-scoped endpoint now gated with `current_user` + ownership check. See `docs/research/m7-basic-auth.md`. **M7b UI (login page)** consolidated with M5b/M6b into one future frontend slice. |
| M7.1| Password reset + email verify | planned  | Needs outbound email provider; small slice after M7. |
| M8  | Projects + flat membership | in progress | Branch `m8/projects`. **Thin slice:** Project entity + flat membership (row existence = member, no roles yet) + nullable `drawings.project_id` + nullable `annotations.author_user_id`. 7 new endpoints (projects CRUD + member add/remove + drawing assignment). Drawing access now: own OR project-member OR unclaimed. Rate-limited member-add (10/min per project+ip). `warnings.flat_membership` flag in detail responses. See `docs/research/m8-projects.md`. |
| M8.1| Invitations + RBAC   | planned  | admin/member/viewer roles scoped to a project; email-based invites (depends on M7.1 email infra). |
| M8.2| Project delete + owner transfer | planned  | Project delete semantics, drawing owner transfer, member-removal-with-data-retention. Depends on M8. |
| M9  | OAuth (Google/GitHub)| planned  | Additive login method; data model unchanged from M7. |
| M10 | Read-only share links | planned  | Tokenized scoped read access; complements M7 auth. |
